 ...
 * =====================================================================================
 *
 *       Filename:  icxxabi.cpp
 *
 *    Description:  
 *
 *        Version:  1.0
 *        Created:  09/18/2012 02:50:35 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
void __cxa_atbegin(){
	...
	unsigned int qtd_ctors=(end_ctors - start_ctors);

	for(unsigned int i=0 ; i<qtd_ctors ; ++i){
		start_ctors[i]();
	}
}
...
void __cxa_finalize(void *function){
	...
	for ( ; i >= 0; --i)
	{
		...
		**/
		if (__atexit_funcs[i].destructor_func == function)
		{
			/* 
